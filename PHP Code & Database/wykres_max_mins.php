...
if(isset($_GET['id_dane']) && isset($_GET['WL_START']) && isset($_GET['WL_STOP'])) 
{
    ...
    $nazwa = $wiersz[1] . " - Min And Max Points";
    ...
    $rezultat = mysqli_query($connection, "SELECT x,y FROM MaxesAndMins WHERE id_dane='$id_dane' AND x>='$WL_START' AND x<='$WL_STOP' ORDER BY id_max_mins");
    $plot = new PHPlot();
    if (!$rezultat) exit();
    $data = array();
...
$rezultat = mysqli_query($connection, "SELECT x,y FROM MaxesAndMins WHERE id_dane='$id_dane' ORDER BY id_max_mins");
$plot = new PHPlot();
if (!$rezultat) exit();
$data = array();
$n_rows = mysqli_num_rows($rezultat);
